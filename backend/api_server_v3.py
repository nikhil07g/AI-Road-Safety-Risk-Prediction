...
def get_weather_api_key() -> str | None:
    """Read the Render variable, with a local frontend .env fallback for development."""
    configured_key = os.getenv(OPENWEATHER_API_KEY_ENV_VAR)
    if configured_key:
        return configured_key

    local_env_path = os.path.join(os.path.dirname(__file__), "..", "frontend", ".env")
    try:
        with open(local_env_path, encoding="utf-8") as local_env:
            for line in local_env:
                if line.startswith("VITE_OPENWEATHER_API_KEY="):
...
async def current_weather(lat: float, lon: float):
    """Fetch weather for a browser-provided location without exposing the API key."""
